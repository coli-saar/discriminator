...
from allennlp.training.metrics import Auc, F1Measure, CategoricalAccuracy, FBetaMeasure
...
@Model.register("transformer_discriminator")
class TransformerClassificationTT(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.true_f1 = F1Measure(positive_label=1)
        self.false_f1 = F1Measure(positive_label=0)
        self.auc = Auc()
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        result = {"acc": self.acc.get_metric(reset), "auc": self.auc.get_metric(reset)}
        result.update(
            {
                f"true_{metric_name}": metrics
                for metric_name, metrics in self.true_f1.get_metric(reset).items()
            }
        )
        result.update(
            {
                f"false_{metric_name}": metrics
                for metric_name, metrics in self.false_f1.get_metric(reset).items()
            }
        )
