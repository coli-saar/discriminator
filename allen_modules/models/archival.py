...
class Archive(NamedTuple):
    """An archive comprises a Model and its experimental configs"""
...
# We archive a model by creating a tar.gz file with its weights, configs, and vocabulary.
#
...
def archive_model(
    ...
    if not os.path.exists(config_file):
        err_msg = f"configs file '{config_file}' does not exist, unable to archive model"
        logger.error(err_msg)
        raise RuntimeError(err_msg)
...
def load_archive(
    ...
    try:
        ...
        # Instantiate model and dataset readers. Use a duplicate of the configs, as it will get consumed.
...
def _parse_version(version: str) -> Tuple[str, str, str]:
    ...
    try:
        ...
        raise ValueError(f"Invalid version '{version}', unable to parser")
