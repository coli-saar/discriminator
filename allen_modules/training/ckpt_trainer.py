...
from allen_modules.evaluation.custmized_evaluator import SimpleEvaluator as Evaluator

logger = logging.getLogger(__name__)


@Trainer.register("ckpt", constructor="from_partial_objects")
class GradientDescentTrainer(Trainer):
    ...
    def _validation_loss(self, epoch: int) -> Tuple[float, Optional[float], int]:
        ...
        try:
            ...
            validation_output_file = Path(f"{self._serialization_dir}/validation_output_e{epoch}.json").open("w",
                                                                                                             encoding="utf-8")
            postprocessor_fn_name: str = "make_output_human_readable"
            model_postprocess_function = getattr(self._pytorch_model, postprocessor_fn_name, None)
            ...
            for batch in val_generator_tqdm:
                ...
                validation_output_file.write(batch_serializer(
                            batch,
    ...
    def from_partial_objects(
        ...
        **kwargs,
        ...
        `FromParams`. If you are not using `FromParams` or configs files, you can safely ignore this
        method.
