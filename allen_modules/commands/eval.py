...
class Eval(Subcommand):
    def add_subparser(self, parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        subparser.add_argument(
            ...
            action="store_true",
            default=False,
            help="log probabilities of predictions and gold",
        )
...
def evaluate_from_archive(
    ...
    # wandb_file = find("*.wandb", "{}/{}".format(archive_file, "wandb/latest-run"))
    # run_id = wandb_file[0].split('/')[-1][4:-6]
    # api = wandb.Api()

    # run = api.run("ykyao/seq2seq_compgen/{}".format(run_id))

    # Load the evaluator from the configs key `Evaluator`
    ...
    for index, evaluation_data_path in enumerate(evaluation_data_path_list):
        ...
        #     data_loader_params = configs.get("data_loader")
        if data_loader_params is None:
            data_loader_params = Params({"type": "simple"})
            if batch_size:
            ...
            if "batch_size" in data_loader_params:
                ...
                data_loader_params["batch_sampler"]["batch_size"] = batch_size
        ...
        # if predictions_output_file_path is not None:
        #     run.upload_file(predictions_output_file_path+'.err.tsv')
